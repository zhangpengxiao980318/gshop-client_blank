...
export default {
  ...
  methods: {
    ...
    async updateOne(shopCart){
      try {
        await this.$store.dispatch('getCheckCart',{skuId:shopCart.skuId,isChecked:shopCart.isChecked ? 0 : 1})
        alert('修改状态成功')
        this.getCartList()
      } catch (error) {
        alert(error.message)
  ...
  computed: {
    ...
    allPrice() {
      return this.shopCartList.reduce((preVal, item) => {
        if (item.isChecked) {
          preVal += item.skuNum * item.cartPrice;
        }
        return preVal;
      }, 0);
    },
    ...
    isAllCheck: {
     ...
     async set(val) {
        try {
          await this.$store.dispatch('getCheckCartAll',val ? 1 : 0)
          alert('修改所有的状态成功')
          this.getCartList()
        } catch (error) {
          alert(error.message)
